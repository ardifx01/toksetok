                ...
                <table class="table table-bordered table-hover align-middle text-center table-dark-mode">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($barang->gambar)
                                <img src="{{ asset('storage/barangs/' . $barang->gambar) }}" alt="{{ $barang->nama_barang }}" width="100">
                                @else
                            ...
                            <td>
                                ...
                                <!-- Tombol Hapus -->
                                <form action="{{ route('barang.destroy', $barang->id) }}" method="POST" class="d-inline ms-2">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-link p-0" onclick="return confirm('Yakin ingin menghapus barang ini?')">
                                    ...
                                    <form action="{{ route('barang.update', $barang->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control @error('nama_barang') is-invalid @enderror" id="nama_barang{{ $barang->id }}" name="nama_barang" value="{{ old('nama_barang', $barang->nama_barang) }}" required>
                                                @error('nama_barang')
                                            ...
                                            <div class="mb-3">
                                                <label for="stok{{ $barang->id }}" class="form-label">Stok</label>
                                                <input type="number" class="form-control @error('stok') is-invalid @enderror" id="stok{{ $barang->id }}" name="stok" value="{{ old('stok', $barang->stok) }}" required>
                                                @error('stok')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
                                            ...
                                            <div class="mb-3">
                                                <label for="gambar{{ $barang->id }}" class="form-label">Gambar Barang</label>
                                                <input type="file" class="form-control @error('gambar') is-invalid @enderror" id="gambar{{ $barang->id }}" name="gambar">
                                                @error('gambar')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
                                                @enderror
                                                <div class="form-text">Opsional. Kosongkan jika tidak ingin mengubah gambar.</div>
                                            </div>
                                            @if ($barang->gambar)
                                            <div class="mb-3">
                                                <img src="{{ asset('storage/barangs/'.$barang->gambar) }}" alt="{{ $barang->nama_barang }}" width="100">
